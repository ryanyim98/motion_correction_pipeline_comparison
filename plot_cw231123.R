...
print(paste("the mean Euclidean norm of run 1 using FSL is: ",mean(r_fs$enorm[r_fsl$run == "run-01"],na.rm = T)))
print(paste("the mean Euclidean norm of run 2 using FSL is: ",mean(r_fs$enorm[r_fsl$run == "run-02"],na.rm = T)))


#fmriprep
r1 <- read.delim("~/Desktop/midmvpa/mid_data/derivatives/sub-02/func/sub-02_task-mid_run-1_desc-confounds_timeseries.tsv", header = T, sep = "\t") %>% 
  mutate_all(~as.numeric(.x)) %>% 
  mutate(run = "run-01")%>% 
  rowwise() %>% 
  mutate(motion_censor = ifelse(sum(motion_outlier00:motion_outlier62) > 0, 1, 0)) %>% 
  select(global_signal:rmsd,trans_x:rot_z_power2,run:motion_censor)

r2 <- read.delim("~/Desktop/midmvpa/mid_data/derivatives/sub-02/func/sub-02_task-mid_run-2_desc-confounds_timeseries.tsv", header = T, sep = "\t")%>% 
  mutate_all(~as.numeric(.x)) %>% 
  mutate(run = "run-02")%>% 
  rowwise() %>% 
  mutate(motion_censor = ifelse(sum(motion_outlier00:motion_outlier52) > 0, 1, 0))%>% 
  select(global_signal:rmsd,trans_x:rot_z_power2,run:motion_censor)
...
ggplot(r_afni, aes(x = tr, y = euc_dist, color = run))+
  ...
  geom_line()+
  ylim(0,max(r_afni$enorm,na.rm = T))+
  ...
  labs(title = "FSL coreg -norm corr")+
  geom_line()+
  ...
  labs(title = "FSL coreg - mutual info")+
  geom_line()+
...
lm.beta::lm.beta(lm(enorm_fs~enorm_fsl_nc,r))
